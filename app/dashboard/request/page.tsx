...
export default function ClientRequestPage() {
  ...
  async function refresh() {
    const u = await apiMe(); setMe(u);
    try { const r = await apiMyTokenRequests(); setRequests(Array.isArray(r?.data) ? r.data : []); }
    catch { setRequests([]); }
  }
  useEffect(() => { refresh(); }, []);
  ...
  async function submit() {
    ...
    try {
      ...
      setQty(10); setMessage(""); await refresh(); toast.success("Token request submitted");
    } catch (e: any) { const msg = e?.message || "Failed"; setErr(msg); toast.error(msg); }
  ...
  return (
    <>
      <style>{`
        ...
        .req-form-row { display:flex; align-items:flex-start; gap:0.75rem; flex-wrap:wrap; }
        ...
        .req-list-empty { display:flex; flex-direction:column; align-items:center; justify-content:center; padding:3rem 1rem; color:#9aa3b0; gap:0.6rem; }
        .req-list-empty svg { opacity:0.3; }
        .req-list-empty-text { font-size:0.85rem; }
      `}</style>

      <div className="clreq-wrap">
        <div><h1 className="clreq-page-title">Token Requests</h1></div>
            ...
            <div className="req-form-row">
              <div className="req-field">
                ...
                <div className="req-input-wrap">
                  <span className="req-input-icon"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="12" r="4"/><circle cx="15" cy="12" r="4"/></svg></span>
                  <input type="number" min={1} value={qty} onChange={e => setQty(parseInt(e.target.value || "1", 10))} className="req-input qty" />
                </div>
              </div>
              <div className="req-field" style={{ flex: 1 }}>
                ...
                <div className="req-input-wrap">
                  <span className="req-input-icon"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></span>
                  <input type="text" placeholder="Reason for request…" value={message} onChange={e => setMessage(e.target.value)} className="req-input msg" style={{ width: "100%" }} />
                </div>
              </div>
              <button disabled={loading || qty < 1} onClick={submit} className="btn-submit">
                {loading ? <span className="spinner" /> : <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>}
          ...
          <div className="req-list">
            {requests.length === 0 ? (
              <div className="req-list-empty">
                <svg width="38" height="38" viewBox="0 0 24 24" fill="none" stroke="#1e3a8a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                  ...
                  <div key={r.id} className="req-list-item">
                    <div className="req-list-left">
                      <div className="req-list-icon">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="12" r="4"/><circle cx="15" cy="12" r="4"/></svg>
                      </div>
                      <div>
                    ...
                    <div className="req-list-right">
                      <span className="status-badge" style={{ background: s.bg, color: s.color, borderColor: s.border }}>
                        <span className="status-dot" style={{ background: s.dot }} />{r.status}
                      </span>
