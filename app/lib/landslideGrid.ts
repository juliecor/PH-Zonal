...
// Landslide hazard rasters live in landslide-data/ with their own manifest.
// Reuses the flood grid shape + sampleGrid; just a separate dir + caches.
type ManifestEntry = { file: string; minLon: number; maxLon: number; minLat: number; maxLat: number };

const DATA_DIR = path.join(process.cwd(), "landslide-data");
export const LANDSLIDE_LABELS: Record<number, string> = { 1: "Low", 2: "Moderate", 3: "High" };
export { sampleGrid };

function getManifest(): ManifestEntry[] {
  const g = globalThis as any;
  const c = g.__LS_MANIFEST__ as { ts: number; entries: ManifestEntry[] } | undefined;
  ...
  g.__LS_MANIFEST__ = { ts: Date.now(), entries };
  return entries;
}

function arrayCache(): Map<string, Promise<FloodGrid | null>> {
  const g = globalThis as any;
  if (!g.__LS_ARRAYS__) g.__LS_ARRAYS__ = new Map();
  return g.__LS_ARRAYS__;
}
...
export async function landslideLevelAcross(lat: number, lon: number): Promise<number | null> {
