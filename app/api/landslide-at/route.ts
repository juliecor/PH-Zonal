...
import { gridsForBounds, landslideLevelAcross, sampleGrid, LANDSLIDE_LABELS } from "../../lib/landslideGrid";
...
export async function POST(req: Request) {
  try {
    ...
    const levels = pts.map((p: any) => {
      ...
      return { level: best, label: best ? LANDSLIDE_LABELS[best] || String(best) : "No landslide" };
    });
    return NextResponse.json({ ok: true, levels });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message ?? "batch landslide failed" }, { status: 500 });
  }
}

// GET ?lat=&lon= → landslide hazard level at a point
export async function GET(req: Request) {
  try {
    ...
    const v = await landslideLevelAcross(lat, lon);
    if (v === null) return NextResponse.json({ ok: true, found: false, inCoverage: false });
    if (!v) return NextResponse.json({ ok: true, found: true, inCoverage: true, level: 0, label: "No landslide" });
    return NextResponse.json({ ok: true, found: true, inCoverage: true, level: v, label: LANDSLIDE_LABELS[v] || String(v) });
  } catch (e: any) {
    return NextResponse.json({ ok: false, error: e?.message ?? "landslide lookup failed" }, { status: 500 });
  }
}
